...
function WebSocketServer(options, callback) {
  ...
  if (options.value.port) {
    // determine http/https based on a passed key
    if (options.value.key !== null) {
        var httpOptions = { 
          key: options.value.key, 
          cert: options.value.cert
        }
        this._server = httpsServer.createServer(httpOptions, function (req, res) {
          res.writeHead(200, {'Content-Type': 'text/plain'});
          res.end('Not implemented');
        });
    } else {
        this._server = httpServer.createServer(function (req, res) {
          res.writeHead(200, {'Content-Type': 'text/plain'});
          res.end('Not implemented');
        });
    }
